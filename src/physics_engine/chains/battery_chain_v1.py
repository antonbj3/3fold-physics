"""Synthetic OCV -> capacity retention -> thermal cascade, with explicit balances."""
import hashlib
import itertools
import json
...
def simulate(parameters=NOMINAL, *, cycles=100, resistance=1.0, chemical=True,
    ...
    while time < horizon and count < 200000:
        ...
        dc = chemical_power*dt
        dl = cooling_power*dt
        temperature += (de+dc-dl)/thermal.MCP
        electrical += de; chemistry += dc; lost += dl
        state[0] -= rates[0]*dt; state[1] -= rates[1]*dt
        state[2] += rates[2]*dt; state[3] -= rates[3]*dt; state[4] += rates[1]*dt
...
def main():
    ...
    (ROOT/"reports/battery_chain_v1.json").write_text(json.dumps(report,indent=2)+"\n")
